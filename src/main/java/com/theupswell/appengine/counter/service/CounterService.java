...
public interface CounterService
{
	 ...
	 * @param skipCache   {@code true} if the cache should be skipped and all shards should be counted in order to get
	 *                    the count; {@code false} if the cache should be consulted first in order to save computing
	 *                    resources while trying to determine the count.
	 ...
	 *                                   Note that despite receiving this exception, it's possible that the datastore
	 *                                   actually committed data properly. Thus, clients should not attempt to retry
	 *                                   after receiving this exception without checking the state of the counter
	 *                                   first.
	 * @throws DatastoreTimeoutException Thrown when a datastore operation times out. This can happen when you attempt
	 *                                   to put, get, or delete too many entities or an entity with too many properties,
	 *                                   or if the datastore is overloaded or having trouble. Note that despite
	 *                                   receiving this exception, it's possible that the datastore actually committed
	 *                                   data properly. Thus, clients should not attempt to retry after receiving this
	 *                                   exception without checking the state of the counter first.
	 */
	void updateCounterDetails(final Counter counter);

	/**
	 * Increment the value of a random shard of a sharded counter by {@code amount} using a random increment {@link
	 * UUID}, which is used to uniquely identify each increment operation.
	 *
	 ...
	 * @throws IllegalArgumentException  if the {@code amount} to increment is negative (increment amounts must always
	 *                                   be positive).
	 * @throws RuntimeException          if the counter exists in the Datastore but has a status that prevents it from
	 *                                   being mutated (e.g., a {@link CounterStatus} of {@code
	 *                                   CounterStatus#DELETING}). Only Counters with a counterStatus of {@link
	 *                                   CounterStatus#AVAILABLE} may be mutated, incremented or decremented.
	 * @throws DatastoreFailureException Thrown when any unknown error occurs while communicating with the data store.
	 *                                   Note that despite receiving this exception, it's possible that the datastore
	 *                                   actually committed data properly. Thus, clients should not attempt to retry
	 *                                   after receiving this exception without checking the state of the counter
	 ...
	 *                                   be positive).
	 * @throws RuntimeException          if the counter exists in the Datastore but has a status that prevents it from
	 *                                   being mutated (e.g., Fa {@link CounterStatus} of {@code
	 *                                   CounterStatus#DELETING}). Only Counters with a counterStatus of {@link
	 *                                   CounterStatus#AVAILABLE} may be mutated, incremented or decremented.
	 * @throws DatastoreFailureException Thrown when any unknown error occurs while communicating with the data store.
	 *                                   Note that despite receiving this exception, it's possible that the datastore
	...
	/**
	 * <p> Decrement the value of a sharded counter by {@code amount} and a random decrement {@link UUID}, which is used
	 * to uniquely identify each decrement operation. </p> <p> Note that this operation will always be performed with no
	 * transactional context. This is because a sharded counter will typically have at least 3 shards, and may have many
	 * more. Thus, in general, decrementing in a transaction would likely exceed exceed the limit of 5 entity groups in
	 * a single TX. As such, this operation should be considered eventually consistent. </p>
	 *
	 ...
	 * @return An instance of {@link CounterOperation} that holds a {@link Set} decrements, as well as the amount that
	 * was actually decremented from this counter. Depending on counter configuration, requests to decrement a counter
	 * by more than its available count will succeed with a decrement amount that is smaller than the requested
	 * decrement amount (e.g., if a counter may not decrement below zero). This return value can be used to discern any
	 * difference between the requested and actual decrement amounts.
	 * @throws NullPointerException      if the {@code counterName} is null.
	 * @throws IllegalArgumentException  if the {@code counterName} is "blank" (i.e., null, empty, or empty spaces).
	 * @throws IllegalArgumentException  if the {@code amount} to decrement is negative (decrement amounts must always
	 *                                   be positive).
	 * @throws DatastoreFailureException Thrown when any unknown error occurs while communicating with the data store.
	 *                                   Note that despite receiving this exception, it's possible that the datastore
	 *                                   actually committed data properly. Thus, clients should not attempt to retry
	...
	/**
	 * @param counterName        A {@link String} representing the name of this counter.
	 * @param shardNumber        The zero-based index of the shard that the requested counterShardOperation occurred
	 *                           in.
	 * @param counterOperationId A {@link UUID} that uniquely identifies the counter operation. This value is supplied
	 *                           by {@link CounterService#increment(String, long, int, UUID)}, or can be autogenerated
	 *                           by appengine-counter via {@link CounterService#increment(String, long)}.
	 * @return
	 */
	...
	/**
	 * Removes a {@link CounterData} from the Datastore and attempts to remove it's corresponding {@link
	 * CounterShardData} entities via a Task Queue. This operation may take some time to complete since it is task queue
	 * based, so constructing or incrementing a {@link CounterData} while the same one is being deleted will fail.
	 *
	 * @throws NullPointerException      if the {@code counterName} is null.
	 * @throws IllegalArgumentException  if the {@code counterName} is "blank" (i.e., null, empty, or empty spaces).
	 * @throws DatastoreFailureException Thrown when any unknown error occurs while communicating with the data store.
